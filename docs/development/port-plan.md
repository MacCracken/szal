...
| `thiserror` | int error-code enum + Result + detail buffer | `enum SzalErr { ERR_NONE = 0; ERR_STEP_FAILED = 1; ... }` + `szal_err_name(code)→cstr` + `set_err_msg/get_err_msg` global buffer (majra `src/error.cyr` is the template; buffer is last-error-wins, NOT thread-safe — per-result error Str on threaded paths) |
| `anyhow` | same machinery; `ERR_OTHER` + detail string | optional richer: sakshi packed errors `sakshi_err_new(code, category)` / `_with_ctx` |
...
| 1 | `src/error.cyr` | error.rs (30 ln) | `enum SzalErr { ERR_NONE=0; ERR_STEP_FAILED; ERR_STEP_TIMEOUT; ERR_FLOW_INVALID; ERR_RETRY_EXHAUSTED; ERR_ROLLBACK_FAILED; ERR_CYCLE; ERR_MIGRATION; ERR_HW_UNAVAILABLE; ERR_QUEUE; ERR_OTHER; }` + `szal_err_name(code)→cstr` + detail-msg buffer `set_err_msg/get_err_msg/clear_err_msg` | majra src/error.cyr is the template |
...
| 17 | `src/engine_hardware.cyr` | engine/hardware.rs (170 ln) | HardwareContext over ai-hwaccel `cached_registry_new(300)`; `hw_check_requirements(steps)` (first unsatisfiable → ERR_HW_UNAVAILABLE); `hw_effective_concurrency(steps, base)` (min 1; currently never called by engine — latent API, port anyway) | dep: ai-hwaccel bundle |
