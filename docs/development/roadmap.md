...
- [x] `src/mcp_tools_*.cyr` (15 files, **54 tools — ALL DONE** ✅ 2026-06-13): encoding (§12), hash (§13), system (§14), json (§15), template (§16), conversion (§17), math (§18), state, step, flow (§19), engine (§20), file (§21), process (§22), git, net (§23). Each group `szal_*`-named with per-group tests; `mcp_tools_net.cyr` defines `all_tools()` (all 15 groups, mod.rs order) + `szal_register_tools()`. Full-stack test asserts all 54 register; `main()` registers all 54 at startup. Security guards (validate_path, no-shell exec + path reject, validate_git_ref, is_safe_url SSRF + pool() rate limits) all ported + tested. **M3 MCP surface complete.** Security checks (SSRF guard, git-ref validation, path confinement, size/count caps) must not regress; each tool is a `mcp_tool_new(mcp_tool_def(...), &handler)` pair, aggregated by `all_tools()` + `szal_register_tools()` in the last tool file. Handler pattern + traps captured in state.md.
...
- [x] `benches/bench_all.bcyr` — emit CSV-compatible rows; `scripts/bench-history.sh` rewrite ✅ 15 benchmarks (the 14 names `benchmarks/history.csv` has tracked since v1.0.1, plus `engine_sequential_10_no_timeout`). Replaces the never-compiling `tests/szal.bcyr` stub — which was in the wrong dir per §1.9 *and* called a `bench()` function that does not exist in `lib/bench.cyr`. Every fixture is pre-checked before timing (22 assertions) so a benchmark can't silently measure an early-error path. `bench-history.sh` now builds/runs the Cyrius harness and parses the harness's machine-readable `BENCHDATA` lines instead of `cargo bench`/criterion (NOT the lossy `bench_report` line — see the script header); `--dry-run` added for CI. **Finding: `engine_sequential_10` is 11.5 ms vs 116 µs for the same flow with `timeout_ms = STEP_I64_MAX` — the `sleep_ms(1)` poll loop in `_run_attempt` (`engine_step_exec.cyr`) is ~99 % of per-step engine cost.** Numbers at version ≥ 2.0.0 are Cyrius; 1.0.1–1.2.0 rows are Rust/criterion — same workload shape, different implementation, so don't read the step between them as a regression.
- [x] `fuzz/*.fcyr` — property harnesses ✅ **5** harnesses, 356 k properties/run, ~1.6 s total: `condition_expr` (tokenizer/parser/evaluator/cache/templates — algebraic laws: De Morgan, precedence, double negation, idempotence), `flow_validate` (cycle detection **differentially checked against an independent Kahn peel-off** written in the harness, + mode/trigger/hierarchical rules + flow JSON round-trip), `step_json` (the §fuzz `step_deser` target — StepDef/StepResult round-trip incl. recursive `sub_steps`, enum wire forms, saturating backoff math), `state_json` (state transitions + the Display-vs-serde split, exhaustive), `hash_uuid` (RFC 1321 vectors + RFC 4122 round-trip). Shared prelude at `fuzz/fuzz_util.cyr` (seeded deterministic PRNG — a failure replays exactly from the printed seed). Covers all four originally-planned targets (step deser, flow deser, flow validate, state transitions) plus condition parsing and md5/uuid. **Validated by mutation testing: 15 deliberately-injected bugs, 15 caught.**
